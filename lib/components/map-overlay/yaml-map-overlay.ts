...
export class YamlMapOverlay extends IMapOverlay {
  ...
  private cachedIcon: HTMLImageElement | undefined;
  private cachedIconSrc: string | undefined;
  private iconLoading: boolean = false;

  private readonly iconDir: string = '/img/solid';
  private readonly iconColor: string = 'white'; // Fixed color for all YamlMapOverlay icons
   ...
   */
  private evaluateFilterExpression(expression: string): boolean {
   ...
   */
  private replaceTempPlaceholder(node: any, replacement: any): any {
    if (!node) return node;
    ...
    if (node.conditions) {
      node.conditions = node.conditions.map((condition: any) =>
        this.replaceTempPlaceholder(condition, replacement)
      );
    }
   ...
   */
  private evaluateFilter(filterNode: any): boolean {
    if (!filterNode) return true;

    switch (filterNode.type) {
      case 'OR':
        return filterNode.conditions.some((condition: any) => this.evaluateFilter(condition));

      case 'AND':
        return filterNode.conditions.every((condition: any) => this.evaluateFilter(condition));
   ...
   */
  private evaluateCondition(key: string, value: string): boolean {
   ...
   */
  private renderIcon(ctx: CanvasRenderingContext2D, x: number, y: number, radius: number, isHighlighted: boolean): void {
   ...
   * @param radius The radius of the overlay
   */
  private renderText(ctx: CanvasRenderingContext2D, x: number, y: number, radius: number, isHighlighted: boolean): void {
   ...
   */
  private calculateOpenStatus(operatingTimes: any): boolean {
    if (!operatingTimes) return false;
    ...
    if (todayHours.open && todayHours.close) {
      const openTime = this.parseTimeString(todayHours.open);
      const closeTime = this.parseTimeString(todayHours.close);
  ...
  private parseTimeString(timeString: string): Date {
    ...
    const hours = parseInt(timeParts[0], 10) + 12 * (timeParts[2] === 'PM' ? 1 : 0);
