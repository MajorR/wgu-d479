import {YamlMapOverlay} from '../map-overlay/yaml-map-overlay.js';
...
export class OverlayInfoPanel {
  ...
  private isTabSwiping: boolean = false;
  ...
  private setupEventListeners(): void {
    // Close button
    const closeBtn = this.container.querySelector('.panel-close-btn') as HTMLElement;
    closeBtn?.addEventListener('click', () => this.hide());

    // Mobile swipe handling
    const handle = this.container.querySelector('.panel-handle') as HTMLElement;
  ...
  private handleTouchMove(e: TouchEvent): void {
    ...
    const scrollContainer = this.container.querySelector('.panel-scroll-container') as HTMLElement;
  ...
  private hasEnoughVerticalSpace(): boolean {
  ...
  private collapseToPartial(): void {
    if (!this.isMobile) return;
  ...
  public show(overlay: YamlMapOverlay): void {
  ...
  private populateContent(overlay: YamlMapOverlay): void {
  ...
  private generateBasicInfo(summary: any, additionalData: any): string {
  ...
  private generateMainTabs(summary: any, additionalData: any): string {
  ...
  private generateOverviewContent(summary: any, additionalData: any): string {
  ...
  private generateOperatingTimes(operatingTimes: any): string {
  ...
  private organizeMoreInfoBySource(moreInfo: any[]): { [source: string]: any[] } {
    const tabs: { [source: string]: any[] } = {};
  ...
  private getTabLabel(source: string): string {
  ...
  private generateTabContent(source: string, items: any[]): string {
  ...
  private generateMoviesContent(items: any[]): string {
    ...
    items.forEach(item => {
      ...
      if (item.resolvedItems && Array.isArray(item.resolvedItems)) {
        ...
        if (item.items && Array.isArray(item.items)) {
          item.items.forEach((originalItem: any) => {
        ...
        item.resolvedItems.forEach((movie: any) => {
  ...
  private generateShowTimes(showtimes: any): string {
    ...
    showtimes.forEach(showtime => {
      ...
      content += `
        <div class="showtime-day">
          ...
          <div class="times">
            ${showtime.times ? showtime.times.map((time: string) => {
  ...
  private isShowtimePast(day: number, timeString: string): boolean {
  ...
  private generateItemList(item: any): string {
  ...
  private switchToNextTab(): void {
    const tabHeaders = this.container.querySelectorAll('.tab-header');
    const currentActiveHeader = this.container.querySelector('.tab-header.active') as HTMLElement;

    if (!currentActiveHeader || tabHeaders.length <= 1) return;
    ...
    const nextHeader = tabHeaders[nextIndex] as HTMLElement;
  ...
  private switchToPreviousTab(): void {
    ...
    const prevHeader = tabHeaders[prevIndex] as HTMLElement;
   ...
   */
  private async loadOverlayIcon(summary: any): Promise<void> {
