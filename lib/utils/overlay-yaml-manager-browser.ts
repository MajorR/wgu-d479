...
export class OverlayYamlManager {
  ...
  private mapOverlaysCategories: any;
  private theatersMovies: any;
  private placeFeatures: any;
  private jsyaml: any;
    ...
    try {
      ...
      if (!this.jsyaml) {
        if (typeof (window as any).jsyaml !== 'undefined') {
          this.jsyaml = (window as any).jsyaml;
        } else {
   ...
   */
  private processMoreInfoItems(infoItem: any, itemCategory: string): void {
    ...
    for (const itemRef of infoItem.items) {
      ...
      if (typeof itemRef === 'string') {
        ...
        if (Array.isArray(lookupDict)) {
          ...
          referencedItem = lookupDict.find((item: any) => {
        ...
        if (uuidKey && infoItem.source === 'theaters-movies') {
          ...
          if (Array.isArray(lookupDict)) {
            ...
            const movieData = lookupDict.find((item: any) => {
          ...
          if (Array.isArray(lookupDict)) {
            referencedItem = lookupDict.find((item: any) => item.id === lookupId);
          } else if (typeof lookupDict === 'object') {
   ...
   */
  private getSourceDictionary(source: string): any {
    switch (source) {
      case 'place-features':
   ...
   */
  public getOverlayInfo(id: string): any {
    // Make sure YAML files are loaded
    if (!this.mapOverlays) {
      throw new Error('YAML files not loaded yet');
    }

    return this.mapOverlays.find((overlay: { id: string }) => overlay.id === id);
  }
   ...
   */
  public getMapOverlaysCategories(): any {
